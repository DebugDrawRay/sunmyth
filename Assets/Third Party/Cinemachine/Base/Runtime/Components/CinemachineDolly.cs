...
namespace Cinemachine
{
    ...
    public class CinemachineDolly : MonoBehaviour, ICinemachineComponent
    {
        ...
        public struct AutoDolly
        {
            ...
            [Tooltip("If checked, will enable automatic dolly, which chooses a path position that is as close as possible to the Follow target")]
            ...
            public AutoDolly(
                bool enabled, float waitTime,  float maxSpeed, 
        ...
        public AutoDolly m_AutoDolly = new AutoDolly(false, 0, 2f, 1, 1);
        ...
        float PerformAutoDolly(float currentPos, float deltaTime)
        {
            ...
            {
                float center = currentPos;
                float radius = 1f;
                int steps = 10;
                if (deltaTime <= 0)
                {
                    radius = center = m_Path.MaxPos / 2;
                    steps = Mathf.RoundToInt(m_Path.MaxPos);
                }
                while (radius >= 0.01f)
                {
                    center = m_Path.GetBestPathPosition(
                        VirtualCamera.Follow.transform.position, 0, //m_AutoDolly.m_TargetDistance, 
                        center, radius, steps);
                    radius /= steps;
                    steps = 10;
                }
